...
/**
 * @class DACController
 * @brief Simulates ESP32 DAC (8-bit)
 */
class DACController {
public:
     ...
     * @param channel DAC channel (0 or 1 for GPIO25/26)
     ...
     */
    uint8_t getValue(uint8_t channel) const { return channels_[channel].value; }

    /**
     * @brief Get output voltage
     */
    float getVoltage(uint8_t channel) const { return channels_[channel].voltage; }
...
private:
    ...
    std::array<DACChannel, 2> channels_;  // DAC1 on GPIO25, DAC2 on GPIO26

    // Statistics
    Stats stats_;

    // Internal
    void updateVoltage(uint8_t channel);
};
